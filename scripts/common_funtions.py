...
def extract_list_cluster_labels(model_dict, selected_models):
    label_lists = []

    for model in selected_models:
        if model in model_dict:
            labels = model_dict[model].labels_.tolist()
            label_lists.append(labels)
        else:
            print(f"Warning: Model '{model}' not found in the dictionary.")
            label_lists.append(None)  # or skip this entry if preferred

    return label_lists

def plot_ari_heatmaps_by_input_type(all_label_lists, model_names_dict = None, font_size = 5):
    """
    Plot a grid of heatmaps showing the pairwise ARI similarity matrices for each input type.
    
    Parameters:
    - all_label_lists: dict mapping input types to lists of clustering label arrays.
    - model_names_dict: optional dict mapping input types to model name lists.
    - font_size: font size for axis labels and annotations.
    """
    n_inputs = len(all_label_lists)
    n_cols = 3
    ...
    for idx, (input_type, label_lists) in enumerate(all_label_lists.items()):
        n = len(label_lists)
        model_names = (
            model_names_dict[input_type] if model_names_dict and input_type in model_names_dict
            else [f"Model {i}" for i in range(n)]
        )
        ...
        ax.set_title(f"{input_type}", fontsize=font_size + 2)
        ax.tick_params(axis='x', labelrotation=90, labelsize=font_size)
    ...
    plt.tight_layout()
    plt.savefig("../figures/pairwise_ari_heatmap.png", dpi = 300, transparent = True, bbox_inches = "tight")
    plt.show()